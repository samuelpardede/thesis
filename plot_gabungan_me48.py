...
X_train, X_test, y_train_scaled, y_test_scaled, train_indices, test_indices = train_test_split(
    X_scaled, y_scaled, data.index, test_size=0.1, random_state=42
)
print("Data siap.")
...
params_rf = {'n_estimators': 221, 'max_depth': 33, 'min_samples_split': 8, 'min_samples_leaf': 5, 'max_leaf_nodes': 121, 'max_features': 'sqrt'}
...
# --- 3. Visualisasi Grafik Aktual vs. Prediksi (Periode 1 Bulan) ---
print("\nMembuat Grafik Aktual vs. Prediksi untuk periode 1 bulan...")
...
# Filter untuk rentang waktu 1 bulan pertama yang tersedia di data uji
if not plot_df.empty:
    start_date = plot_df.index.min()
    end_date = start_date + pd.DateOffset(months=1)
    plot_event_df = plot_df.loc[start_date:end_date]
    
    # --- PERUBAHAN DI SINI: Reset index agar sumbu-X menjadi urutan angka ---
    plot_event_df.reset_index(drop=True, inplace=True)
    # --------------------------------------------------------------------

    print(f"Menampilkan plot untuk periode 1 bulan, dimulai dari {start_date.date()}")
    ...
    plt.title(f'Grafik Aktual vs. Prediksi - Periode 1 Bulan (Data ME 48)', fontsize=16)
    # --- PERUBAHAN DI SINI: Label sumbu-X diubah ---
