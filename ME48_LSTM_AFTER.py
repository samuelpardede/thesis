...
# 4. Ubah ke format LSTM (3D)
X_lstm = X_scaled.reshape((X_scaled.shape[0], 1, X_scaled.shape[1]))

# 5. Split Data
X_train, X_test, y_train, y_test = train_test_split(X_lstm, y_scaled, test_size=0.1, random_state=42)

# 6. Model LSTM dengan Hasil Tuning
model = Sequential()
model.add(LSTM(units=32, input_shape=(X_train.shape[1], X_train.shape[2]), dropout=0.0))
model.add(Dense(1))

optimizer = Adam(learning_rate=0.003081373838058736)
...
print("=== Evaluasi Model LSTM (Tuning) ===")
print(f"[Train] RMSE: {np.sqrt(mean_squared_error(y_train_inv, train_preds_inv)):.3f} | MAE: {mean_absolute_error(y_train_inv, train_preds_inv):.3f}")
print(f"[Test ] RMSE: {np.sqrt(mean_squared_error(y_test_inv, test_preds_inv)):.3f} | MAE: {mean_absolute_error(y_test_inv, test_preds_inv):.3f}")

# 8. Visualisasi Prediksi
plt.figure(figsize=(12, 6))
plt.plot(y_test_inv, label='Aktual')
plt.plot(test_preds_inv, label='Prediksi LSTM')
plt.title('Prediksi vs Aktual Curah Hujan (LSTM)')
